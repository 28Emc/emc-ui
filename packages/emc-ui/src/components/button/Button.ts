...
const VARIANT_CLASSES: Record<ButtonVariant, string> = {
  ...
  outline: 'border border-brand-500 text-brand-600 hover:bg-brand-50 dark:border-brand-400 dark:text-brand-400 dark:hover:bg-brand-500/10',
...
export class EmcButton extends LitElement {
  ...
  static styles = css`
    ...
    button {
      ...
      white-space: nowrap;
      cursor: pointer;
      transition: background-color 150ms, color 150ms, border-color 150ms, box-shadow 150ms, filter 150ms, transform 150ms;
      outline: none;
    }
  ...
  render() {
    ...
    const spinnerSize = this.size === 'sm' || this.size === 'icon-sm' ? 14 : this.size === 'lg' ? 18 : 16;
    ...
    return html`
      ...
      >
        ${this.loading ? html`
          <svg class="spinner h-${spinnerSize} w-${spinnerSize} text-current" viewBox="0 0 24 24" fill="none" aria-hidden="true">
